...
@Deprecated
public class ActionsQuery extends DatabaseQuery<List<ActionsTable>> {
    private final ActionParameterData parameterData;

    public ActionsQuery(Database database, Plugin plugin) {
        super(database, plugin);

        parameterData = new ActionParameterData(getConnection());
    }

    public ActionParameterData getParameterData() {
    ...
    protected PreparedStatement prepare() throws SQLException {
        return parameterData.createQueryString();
    }
    ...
    protected List<ActionsTable> createWrapper(ResultSet queryRes) {
        try{
            ...
            do {
                ...
                if (parameterData.getSelectData().isGettingActionName()){
                    actionBuilder = new ActionBuilder(queryRes.getString("Action_Name"));
                }else{
                    actionBuilder = new ActionBuilder(parameterData.getWhereData().getName());
                }

                if (parameterData.getSelectData().isGettingRespondingAction()){
                    actionBuilder.setRespondingAction(queryRes.getString("Responding_Action"));
                }

                if (parameterData.getSelectData().isGettingUsagePermission()){
                    actionBuilder.setPermission(queryRes.getString("Usage_Permission"));
                }

                if (parameterData.getSelectData().isGettingDefaultDuration()){
